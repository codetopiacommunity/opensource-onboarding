# Getting Started Guides

This directory contains the core onboarding guides to help you start contributing to open source.

## The Curriculum

Twelve guides, numbered 00 to 11. The numbering starts at zero because
that is how the files are named, so guide 00 is the first one and guide
11 is the last. Each guide carries links at the top and bottom to
move backwards and forwards, and a bar showing how far through the
course you are.

Work through these guides in order:

0. [What Is Open Source?](./00-what-is-open-source.md)
1. [Your Computer and the Terminal](./01-your-computer-and-the-terminal.md)
2. [Installing and Configuring Git](./02-installing-and-configuring-git.md)
3. [Your First Commit](./03-your-first-commit.md)
4. [GitHub Account and SSH](./04-github-account-and-ssh.md)
5. [Your First Pull Request](./05-your-first-pull-request.md)
6. [Reviewing a Pull Request](./06-reviewing-a-pull-request.md)
7. [Finding and Claiming an Issue](./07-finding-and-claiming-an-issue.md)
8. [Resolving a Merge Conflict](./08-resolving-a-merge-conflict.md)
9. [Keeping Your Fork in Sync](./09-keeping-your-fork-in-sync.md)
10. [Reporting a Good Issue](./10-reporting-a-good-issue.md)
11. [Finding Your First Real Project](./11-finding-your-first-real-project.md)

---
## Need help?

If you are stuck, refer to the [HELP.md](../HELP.md) guide in the root directory for instructions on how to get assistance via GitHub Discussions.
